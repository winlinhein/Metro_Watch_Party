...
<div x-show="showReportRoomModal"
    ...
    <div class="absolute inset-0 bg-black/80 backdrop-blur-sm"
         x-show="showReportRoomModal"
         x-transition.opacity
         @click="closeReportRoomModal()"></div>
    ...
    <div class="relative w-full max-w-sm bg-[#0a0a0f] border border-red-500/30 rounded-2xl shadow-2xl p-6"
        ...
        <p class="text-xs text-white/50 mb-4">
            Reporting <span class="font-bold text-white" x-text="roomName"></span>
            <span x-show="currentMovie?.title"> · watching <span class="text-white" x-text="currentMovie?.title"></span></span>.
        </p>

        <div class="space-y-3 mb-5">
            <p class="text-sm text-gray-400">Select reasons:</p>
            <div class="flex flex-wrap gap-2">
                <template x-for="reason in availableReasons" :key="'room-reason-'+reason.reason_id">
                    <label
                        ...
                        :class="selectedRoomReasonIds.includes(String(reason.reason_id)) || selectedRoomReasonIds.includes(Number(reason.reason_id))
        ...
        <div class="mb-4">
            ...
            <textarea x-model="reportRoomDescription" placeholder="Describe what happened in this room..." rows="3"
        ...
        <div class="flex gap-3 mt-4">
            <button @click="closeReportRoomModal()" class="flex-1 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-white/70 font-semibold transition-colors">Cancel</button>
            <button @click="submitRoomReport()"
                    ...
                    :disabled="selectedRoomReasonIds.length === 0 && !reportRoomDescription.trim()">
                Submit
            </button>
        </div>
    </div>
