...
<div x-data="{
    ...
    get filteredUsers() {
        return this.usersList.filter(u => 
            ...
            (u.name.toLowerCase().includes(this.searchQuery.toLowerCase()) || u.email.toLowerCase().includes(this.searchQuery.toLowerCase()))
        );
    },
    openBanModal(user) {
        this.userToBan = user;
    ...
    <div class="flex items-center justify-between mb-10 stagger-item">
        ...
        <div class="flex gap-4 items-center">
            ...
            <select x-model="roleFilter" class="bg-[#030305] border border-white/10 rounded-xl px-4 py-2 text-sm text-white outline-none cursor-pointer focus:border-blue-500/50 transition-colors">
                ...
                <option class="bg-[#030305] text-white" value="Standard">Standard</option>
    ...
    <div class="glass-card rounded-2xl overflow-hidden stagger-item border-white/10 relative min-h-[400px]">
        <div class="overflow-x-auto"><table class="w-full text-left border-collapse whitespace-nowrap">
            <thead>
            ...
            <tbody>
                <template x-for="(user, index) in filteredUsers" :key="user.id">
                    <tr class="border-b border-white/5 hover:bg-white/[0.03] transition-colors group user-row" :style="`animation-delay: ${index * 0.1}s`">
                        <td class="p-5 flex items-center gap-4">
                            <div class="relative">
                                <img :src="`https://ui-avatars.com/api/?name=${user.name}&background=random&color=fff&bold=true`" class="w-10 h-10 rounded-xl border border-white/10 group-hover:border-white/30 transition-colors">
                                <div class="absolute -bottom-1 -right-1 w-3.5 h-3.5 rounded-full border-2 border-[#030305] transition-colors"
                                      :class="user.status === 'Online' ? 'bg-green-500 group-hover:shadow-[0_0_8px_#22c55e]' : (user.status === 'Banned' ? 'bg-red-500' : 'bg-gray-600')"></div>
                            </div>
                            <div>
                        ...
                        <td class="p-5">
                            <span class="px-3 py-1 rounded-md text-xs font-bold border"
                                   :class="{
                                       'bg-green-500/10 text-green-400 border-green-500/20': user.status === 'Online',
                                       'bg-red-500/10 text-red-400 border-red-500/20': user.status === 'Banned',
                                       'bg-white/5 text-white/40 border-white/10': user.status !== 'Online' && user.status !== 'Banned'
                                   }"
                        ...
                        <td class="p-5">
                            <span class="text-white/80 text-sm font-medium flex items-center gap-2">
                                <span class="material-symbols-outlined text-[16px]"
                                       :class="user.role === 'Premium' ? 'text-indigo-400' : (user.role === 'Moderator' ? 'text-orange-400' : 'text-white/30')"
                                      x-text="user.role === 'Premium' ? 'star' : (user.role === 'Moderator' ? 'security' : 'person')"></span>
        ...
        <div class="relative bg-[#0a0a0f] border border-red-500/30 rounded-2xl p-8 max-w-md w-full shadow-[0_0_40px_rgba(239,68,68,0.15)]"
             ...
             <template x-if="userToBan">
                 <div class="bg-white/5 border border-white/10 rounded-xl p-4 mb-6 flex items-center gap-4">
                     <img :src="`https://ui-avatars.com/api/?name=${userToBan.name}&background=random&color=fff&bold=true`" class="w-10 h-10 rounded-lg">
                     <div>
             ...
             <div class="mb-6">
                 ...
                 <textarea class="w-full bg-[#030305] border border-white/10 rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-red-500/50 transition-colors h-24 resize-none" placeholder="Provide evidence or internal notes..."></textarea>
             </div>
