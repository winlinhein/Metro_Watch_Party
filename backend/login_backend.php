...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        // FIX: Added `role_id` to SELECT statement
        $stmt = $conn->prepare("SELECT user_id, role_id, hashed_password, status FROM users WHERE email = :email");
        ...
        if ($user && password_verify($user_pass, $user['hashed_password'])) {
            ...
            // FIX: Safely retrieve the role AFTER validating user existence
            $roleID = $user['role_id'];
            $user_type = $conn->prepare("SELECT role FROM roles WHERE role_id = :roleID");
            $user_type->execute([':roleID' => $roleID]);
            $type = $user_type->fetch(PDO::FETCH_ASSOC); // FIX: Changed $stmt to $user_type
            $role = $type['role'] ?? 'user';
